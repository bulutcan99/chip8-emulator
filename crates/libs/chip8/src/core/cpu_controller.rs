...
impl CpuController {
	...
	fn skip_next_instruction(&self, mem_ctr: &mut Emulator) {
		mem_ctr.inc_pc_by(2)
	}
	...
	fn return_from_subroutine(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
	...
	fn jump_to_address(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
	...
	fn call_address(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let address = self.extract_12bit_address();
	...
	// 3XNN - SE VX
	fn skip_equal_vx_byte(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
	...
	// 4XNN - SNE VX
	fn skip_not_equal_vx_byte(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let v = mem_ctr.get_v(x)?;
		let second_byte = self.second_byte();
		if v != second_byte {
	...
	// 5XY0 - SE VX == VY
	fn skip_equal_vx_vy(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
	...
	fn load_vx_with_byte(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let second_byte = self.second_byte();
	...
	// 7XNN - ADD VX, NN
	fn add_vx_with_byte(&mut self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
	...
	// 8XY0 - LD VX with VY
	fn load_vx_with_vy(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let y = self.x();
		let vy = mem_ctr.get_v(y)?;
		mem_ctr.set_v(x, vy)?;
	...
	fn load_vx_with_vx_or_vy(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let y = self.y();
		let vx = mem_ctr.get_v(x)?;
		let vy = mem_ctr.get_v(y)?;
	...
	fn load_vx_with_vx_and_vy(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let y = self.y();
		let vx = mem_ctr.get_v(x)?;
		let vy = mem_ctr.get_v(y)?;
	...
	fn load_vx_with_vx_xor_vy(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let y = self.y();
		let vx = mem_ctr.get_v(x)?;
		let vy = mem_ctr.get_v(y)?;
	...
	// 8XY4 - ADD VX, VY
	fn add_vx_with_vy(&mut self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let y = self.y();
	...
	// 8XY5 - SUB VX, VY
	fn sub_vx_to_vy(&mut self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let y = self.y();
	...
	fn shift_vx_to_right(&mut self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let vx = mem_ctr.get_v(x)?;
	...
	// 8XY7 - MINUS VX = VY-VX
	fn minus_vx_from_vy(&self, mem_ctr: &mut Emulator) -> Result<(), Error> {
		let x = self.x();
		let vx = mem_ctr.get_v(x)?;
